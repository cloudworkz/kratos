...
const fs = require("fs");
const debug = debugBase("service:bin");
...
if (program.logs) {
  debugBase.enable("service:*");
} else {
  debugBase.enable("service:bin");
}

const defaultOptions = require("./baseConfig.js");

if (!program.args || !program.args.length) {
  debug("No config file path passed, exiting. Make sure to pass it as first argument after options.");
...
debug(`service in version`, pjson.version);
...
service.run().then(() => {
  ...
  readAndDisplayBanner().then(() => {
    debug(`service is ready to accept connections.`);
  });
}, error => {

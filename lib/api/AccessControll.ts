...
const debug = Debug("service:access");
...
export default class AccessControll {
  ...
  private wildcardAccessAllowedForToken(token: string | null): boolean {
    ...
    if (!token) {
      debug("Access not allowed for token", "token", "reason: no token.");
    ...
    if (!configuration) {
      debug("Access not allowed for token", "token", "reason: no configuration.");
      this.metrics.inc("access_bad");
      return false;
    }
    ...
    debug("Access not allowed for token", "token", "reason: no wildcard.");
    this.metrics.inc("access_bad");
    return false;
  }
